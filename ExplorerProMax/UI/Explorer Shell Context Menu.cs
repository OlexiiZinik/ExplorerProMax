...
namespace Peter
{
    ...
    public class ShellContextMenu : NativeWindow
    {
        ...
        /// Get the PIDLs
        /// </summary>
        /// <param name="arrFSI">Array of FileSystemInfo</param>
        /// <returns>Array of PIDLs</returns>
        protected IntPtr[] GetPIDLs(FileSystemInfo[] arrFSI)
        {
            ...
            if (arrFSI[0] is FileInfo)
                oParentFolder = GetParentFolder((arrFSI[0] as FileInfo).DirectoryName);
            else
                oParentFolder = GetParentFolder((arrFSI[0] as DirectoryInfo).Parent.FullName);
            ...
            int n = 0;
            foreach (FileSystemInfo fi in arrFSI)
            {
                // Get the file relative to folder
                uint pchEaten = 0;
                SFGAO pdwAttributes = 0;
        ...
        public void ShowContextMenu(FileSystemInfo[] fileSystemInfos, Point pointScreen)
        {
            // Release all resources first.
            ReleaseAll();
            _arrPIDLs = GetPIDLs(fileSystemInfos);
            this.ShowContextMenu(pointScreen);
        }
